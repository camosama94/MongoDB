...
//Creamos el modelo
const ordenador = mongoose.model('ordenador', ordenadoresSchema,
    'ordenadores');
const buscaPrimero = () => {
...
const buscaPrecioMayor = (parametro) => {
    ...
    ordenador.find({ precio: { $gt: parametro } })
        .then(ordenadores => {
            if (ordenadores.length > 0) {
                console.log('Ordenadores encontrados con precio mayor que ',
                    parametro, ' encontrados: ', ordenadores);
...
const actualizaPrecio = (idOrdenador, nuevoPrecio) => {
    ordenador.findByIdAndUpdate(idOrdenador, { precio: nuevoPrecio }, {
        new: true
    })
        ...
        .catch(err => console.error('Error al actualizar el ordenador:',
            err));
}
const borraOrdenador = (idOrdenadorParaBorrar) => {
    ...
    return ordenador.findByIdAndDelete(idOrdenadorParaBorrar)
        ...
        .catch(err => {
            console.error('Error al eliminar el ordenador:',
                err); throw err;
        });
}
...
//el tercer parametro ( { new: true } ) es para que devuelva el
...
module.exports = {
    ...
    buscaPrecioMayor, creaNuevoOrdenador, actualizaPrecio, borraOrdenador,
    ordenador
};